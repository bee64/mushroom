...
* idk if windows build works. search `TODO: Windows` to see things i'm unsure about

## Other ways to run it
* run web build: `trunk serve` 
...
### Updating the icons
 1. Replace `build/icon_1024x1024.png` is the main icon. When changed, run `create_icns.sh` or `create_icns_linux.sh` if you use linux (make sure to run the script inside the `build/macos` directory) - _Note: `create_icns.sh` requires a mac, and `create_icns_linux.sh` requires imagemagick and png2icns_
